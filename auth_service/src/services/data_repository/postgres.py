...
class PostgresService(PostgresCrudService):
    ...
    async def create_login_history(
        ...
        model_instance = LoginHistory(
            user_id=user_id, ip_address=ip_address, refresh_token=refresh_token, device=device
        )
    ...
    async def get_oauth_provider(self, provider_name: str, provider_user_id: str) -> OAuthProvider | None:
        """Retrieve an OAuth provider."""
        async with self.session.begin():
            query = select(OAuthProvider).where(
                OAuthProvider.provider_name == provider_name, OAuthProvider.provider_user_id == provider_user_id
            )
